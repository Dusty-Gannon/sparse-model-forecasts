####################################################
# Fitting filtered Beverton-Holt models to spatially
# explicit simulated community data
####################################################
...
# function to fit the model
  fit_filtered_BH <- function(
    ...
    # convert cover and fecundity data to wide format
    sp <- unique(as.character(dat$subplot_fecundity$species))
    fec_wide <- matrix(
      data = unique(dat$subplot_fecundity$t),
      ncol = 1
    )
    cover_wide <- matrix(
      data = unique(dat$subplot_cover$t),
      ncol = 1
    )
    for(s in sp){
      fec_wide <- cbind(
        fec_wide,
        subset(dat$subplot_fecundity, species == s)$fecundity
      )
      cover_wide <- cbind(
        cover_wide,
        subset(dat$subplot_cover, species == s)$cover
      )
    }
    percent_cover_wide <- cbind(
      cover_wide[, 1],
      cover_wide[, -1] * sub_cells *sub_cells
    )
    colnames(fec_wide) <- c("t", sp)
    fec_df <- as.data.frame(fec_wide)
    colnames(percent_cover_wide) <- c("t", sp)
    cover_df <- as.data.frame(percent_cover_wide)
    ...
    col_ids_a <- which(names(fec_df) %in% ann)
    foc_a <- myTryCatch(foc_a <- choose_focal(
      df = fec_df,
      col_ids = col_ids_a, num_ngs = 2,
      exclude_names = c("t", "env")
    ))

    # find a perennial with high abundance and at least one
    #  strong competitor
    col_ids_p <- which(!(names(fec_df) %in% c(ann, "t", "env")))
    foc_p <- myTryCatch(choose_focal(
      df = fec_df,
      col_ids = col_ids_p, num_ngs = 2,
      exclude_names = c("t", "env")
    ))

    # define time steps
    if(!is.null(foc_a$value)){
      tsteps_p1_a <- foc_a$value$tw
      tsteps_m1_a <- foc_a$value$tw - 1
    }
    if(!is.null(foc_p$value)){
      tsteps_p1_p <- foc_p$value$tw
      tsteps_m1_p <- foc_p$value$tw - 1
    }

    ### Fitting the model for the annual focal species ###
    if(!is.null(foc_a$value)){
      tau_0_a <- tau0(
        y = fec_df[tsteps_m1_a, foc_a$value$foc],
        m0 = 2,
        M = ncol(fec_df) - 2,
        N = n_obs,
        fam = "poisson"
      ...
      X_beta_a <- as.matrix(
        cover_df[tsteps_m1_a, -which(colnames(cover_df) %in% c("t", "env", foc_a$value$foc))]
      )
      ...
      X_alpha_a <- cbind(
        rep(1, n_obs),
        cover_df$env[tsteps_m1_a],
        (cover_df$env[tsteps_m1_a])^2
      )
      ...
      datlist_a <- list(
        ...
        P0 = ncol(X_alpha_a),
        P = ncol(X_beta_a),
        fec = as.integer(fec_df[tsteps_m1_a, foc_a$value$foc]),
        vm = as.integer(cover_df[tsteps_m1_a, foc_a$value$foc]),
        vp = as.integer(cover_df[tsteps_p1_a, foc_a$value$foc]),
        X_alpha = X_alpha_a,
        X_beta = X_beta_a,
        tau0 = tau_0_a,
        slab_scl = 1,
      ...
      for(i in 1:nchains){
        inits[[i]] <- list(
          alpha_std = rnorm(ncol(X_alpha_a)),
          beta_std = rnorm(ncol(X_beta_a)),
          beta_ii_std = rnorm(1),
          local_scl = rexp(ncol(X_beta_a)),
          tau_std = rexp(1),
          p = runif(1),
          lambda_d = rexp(1, rate = datlist_a$b_d)
        )
      }
    ...
    if(!is.null(foc_p$value)){
      tau_0_p <- tau0(
        y = fec_df[tsteps_p1, foc_p$value],
        m0 = 2,
        M = ncol(fec_df) - 2,
        N = n_obs,
        fam = "poisson"
      ...
      # define the model matrix
      X_beta_p <- as.matrix(
        fec_df[tsteps_m1, -which(colnames(fec_df) %in% c("t", "env"))]
      )
      ...
      # compile data list for fitting the model
      y_star <- as.integer(fec_df[tsteps_p1, foc_p$value])
      y_star[y_star == 0] <- 0.5
      ...
      datlist_p <- list(
        ...
        P0 = ncol(X_alpha),
        P = ncol(X_beta_p),
        y = as.integer(fec_df[tsteps_p1, foc_p$value]),
        y_star = y_star,
        X_alpha = X_alpha,
        X_beta = X_beta_p,
        tau0 = tau_0_p,
...
### fitting the models ###
  ...
  dat_list <- readRDS(
    here("Data/terrestrial_sim_data/simdat_500reps_500steps_S2s48_5ann_env0.1.rds")
  )

  # compile jags model
  mod <- here("JAGS/filtered_Pois_BevHolt_FHS.jags")
