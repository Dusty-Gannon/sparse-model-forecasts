...
##### Wrapper function to fit and summarize #####
  fit_2_summaries <- function(X, stan_mod, tsteps = 51:100, pip = 0.9){

    N_list <- list(N_full = X$N_full, N_cor = X$N_cor)
...
##################################
  ...
  saveRDS(results, file = here(
    "Data/terrestrial_sim_data/lnorm_ricker/vr_tests_1param.rds"
  ))
