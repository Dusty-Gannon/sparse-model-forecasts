...
parameters{
  ...
  real alpha_std;                // unshrunk coefficients (self-limitation and intercept)
...
transformed parameters{
  ...
  vector[N] mu;                // declare vector of linear predictors
  ...
  for(i in 2:N){
    ...
    z[i] = phi * (log(y_star[i-1]) - alpha) + theta * log(y_star[i-1] / mu[i-1]);
    mu[i] = exp(alpha + z[i]);
