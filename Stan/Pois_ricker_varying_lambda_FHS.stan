...
// The time-varying growth rate, lambda, is unknown, but some
// prior data exist such that an informative prior can be used.
///////////////////////////////////////////////////////////////
...
data {
  ...
  real<lower = 0> a;            // prior shape parameter for prior on lambda
  real<lower = 0> b;            // prior shape parameter for prior on lambda
  int<lower = 0> y[N];          // vector of responses
  real<lower = 0> lambda_scl;   // scale of lambda max
...
parameters{
  ...
  vector<lower = 0>[N] lambda_std;     // intrinsic growth
...
transformed parameters{
  ...
  //scale lambda
  vector[N] lambda = lambda_scl * lambda_std;

  // construct linear predictors
  eta[1] = log(y[1]);
  for(t in 2:N){
    // mean   time-varying growth         intra                     generic                       non-generic          offset
    eta[t] = log(lambda[t - 1]) + (y[t - 1]/scl_X) * alpha + (X_beta0[t - 1, ] * ones) * beta0 + X_beta[t - 1, ] * beta + log(y[t - 1]);
