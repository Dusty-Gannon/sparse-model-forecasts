...
data {
  ...
  vector<lower = 0>[N] lambda;     // known lambdas
  ...
  real<lower = 0> beta0_scl;    // a priori guess for the scale of a generic competitor
...
transformed parameters{
  ...
  eta[1] = log(y[1]);
  for(t in 2:N){
    // mean       intrinsic growth          intra                     generic                       non-generic          offset
