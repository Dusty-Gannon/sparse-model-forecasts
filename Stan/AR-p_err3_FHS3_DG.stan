...
// priors. The horseshoe for phi is placed on the partial
// correlations r_phi, which are mapped to AR coefficients
...
parameters{
  ...
  vector[p] r_phi_std;        // unconstrained auxiliary for partial correlations (non-centered)
...
transformed parameters{
  ...
  real mu;               // declare vector of means
  vector[N] err;              // vector of residuals
  vector[P] beta;             // all scaled regression coefficients
  vector[p] r_phi;            // partial correlations
  vector[p] phi;              // AR coefficients (from Levinson-Durbin)
  ...
  r_phi = tanh(tau_phi * local_scale_tilde_p .* r_phi_std);

  // Levinson-Durbin recursion: partial correlations -> AR coefficients
  {
    matrix[p, p] P = diag_matrix(r_phi);
    for(k in 2:p){
      for(i in 1:(k - 1)){
