...
simulate_Ricker_comp_communities <- function(X, sim_params, vary_comp = FALSE){
  ...
  if(isTRUE(vary_comp)){
    ...
    env[1] <- rnorm(1, sd = sim_params$sigma_env[1]/sqrt(1 - phi^2))

    # generate matrix of parameters that determine competition based
    #  on number of non-generics but NO environmental covariates
    B <- ricker_comp_array(
      nsp = nsp, num_ngs = sim_params$S,
  ...
  for(t in 1:(sim_params$steps-1)){
    ...
    if(isTRUE(vary_comp)){
      ...
      lambda_t <- gauss_env_effect(env[t], lambda_max, optims = sp_optims, tau = tau)
    }
    ...
    if(isTRUE(vary_comp)){
      ...
      env[t + 1] <- phi * env[t] + rnorm(1, sd = sim_params$sigma_env[1])
    }
...
#### Run the simulations and store datasets ####
  ...
  # define whether competition should vary with the environment
  if(is.na(args[3])){
    vary_comp <- FALSE
  } else{
    vary_comp <- TRUE
  ...
  stopCluster(cl)
  fname <- paste0(
    "simdat_", nsims, "reps_", sim_params$steps, "steps_S",
